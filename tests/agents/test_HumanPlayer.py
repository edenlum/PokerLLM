...
class TestHumanPlayer(unittest.TestCase):
    ...
    def test_fold_action(self, mock_input):
        action, amount, is_fallback = self.player.get_action("Test game history", ['fold', 'call', 'raise'], 10)
    ...
    def test_call_action(self, mock_input):
        ...
        self.assertEqual(amount, 10)  # amount should equal amount_to_call
        self.assertFalse(is_fallback)
    ...
    def test_bet_action(self, mock_input):
        action, amount, is_fallback = self.player.get_action("Test game history", ['check', 'bet'], 0)
